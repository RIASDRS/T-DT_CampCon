...
// 转换函数
std::vector<cv::Point3f> eigenToCvPoint3f(const std::vector<Eigen::Vector3d> &eigen_points)
{
    std::vector<cv::Point3f> cv_points;
    cv_points.reserve(eigen_points.size());

    for (const auto &eigen_pt : eigen_points)
    {
        cv_points.emplace_back(static_cast<float>(eigen_pt.x()), static_cast<float>(eigen_pt.y()), static_cast<float>(eigen_pt.z()));
    }
...
int main(int argc, char *argv[])
{
    ...
    {             // 在config—loader中的loadFromCommmandLine抓取命令行传递的配置参数
        return 0; // 用户请求帮助信息
    }
    ...
    // 1. 初始化EKF参数
    // --------------------------
    float64 chassis_length = 0.8; // 小车底盘长度（单位：米，根据实际测量值修改）
    float64 chassis_width = 0.4;  // 小车底盘宽度（单位：米，根据实际测量值修改）
    // float64 fps = 30.0;             // 摄像头帧率（单位：Hz，如30/60）
    float64 dt = 1.0 / fps; // 采样时间（单位：秒，1/帧率）

    // 创建EKF参数对象
    ArmorEKFParams ekf_params(chassis_length, chassis_width, dt);
    // 创建EKF实例
    ArmorEKF ekf(ekf_params);
    ...
    while (true)
    {
        ...
        for (const auto &detection_result : results) // 遍历vector<DetectionResult>
        {
            ...
            for (const auto &center_point : pose.center_point)
            {
                ...
                //std::cout << center_point.x << "," << center_point.y << "," << center_point.z << std::endl;                    // 为二项式拟合加入点
                observations.push_back({Eigen::Vector3d(center_point.x, center_point.y, center_point.z), ArmorType::UNKNOWN}); // 为EKF加入点
            }
        } // 在这里你可以得到装甲板中心点坐标。
        ...
        // 3. EKF核心步骤
        // --------------------------
        ekf.predict(); // 预测步骤（基于上一帧状态）

        if (!observations.empty())
        {
            ekf.update(observations); // 更新步骤（使用当前帧观测）
            std::printf("EKF Updated!\n");
        }

        // --------------------------
        // 4. 获取EKF输出结果
        // --------------------------
        // 结果1：底盘中心的8维状态（x, y, z, vx, vy, vz, omega, alpha）
        Eigen::Matrix<double, 8, 1> chassis_state = ekf.getState();
        // 可按需提取：
        double center_x = chassis_state(0); // 底盘中心X坐标（m）
        double center_y = chassis_state(1); // 底盘中心Y坐标（m）
        double center_z = chassis_state(2); // 底盘中心Z坐标（m）
        double omega = chassis_state(6);    // 旋转角速度（rad/s）

        // 结果2：4个装甲板的位置估计（相机坐标系下）
        std::vector<Eigen::Vector3d> armor_positions = ekf.getArmorPositions();
        // 顺序：[0]前装甲板, [1]后装甲板, [2]左装甲板, [3]右装甲板
        Eigen::Vector3d front_armor = armor_positions[0]; // 前装甲板坐标（x,y,z）
        Eigen::Vector3d back_armor = armor_positions[1];
        Eigen::Vector3d left_armor = armor_positions[2];
        Eigen::Vector3d right_armor = armor_positions[3];

        std::vector<cv::Point3f> cv_armors = eigenToCvPoint3f(armor_positions);
        project3d(display_frame, cv_armors);
